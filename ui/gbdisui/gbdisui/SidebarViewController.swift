...
class SidebarViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    disassembledSubscriber = NotificationCenter.default.publisher(for: .disassembled)
      .receive(on: RunLoop.main)
      .sink(receiveValue: { [weak self] notification in
        guard let self = self else {
          return
        }
        self.dataSource.apply(self.disassembledSnapshot(files: notification.userInfo!.keys.map { $0 as! String }),
                              to: .disassembled, animatingDifferences: true)
      })
  }
}
