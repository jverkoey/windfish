...
extension Disassembler.BankWorker {
  func rewriteRunGroup(_ runGroup: RunGroup) {
    ...
    for run: Disassembler.Run in runGroup {
      guard run.startLocation.bankIndex == bank else {
        continue
      }
    ...
    assert(contiguousScope.lowerBound.bankIndex == bank)
  ...
  private func propagateTypes(for run: Disassembler.Run) {
    ...
    LR35902.trace(range: visitedRange, cartridgeData: context.cartridgeData, disassembler: router!) { (instruction: LR35902.Instruction, currentLocation: Cartridge.Location, cpu: LR35902, memory: TraceableMemory) in
