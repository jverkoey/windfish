...
extension LR35902 {
  ...
  public func advance(memory: AddressableMemory) {
    ...
    if isRunning {
      ...
      } else if nextAction == .continueExecution, let instructionEmulator = machineInstruction.instructionEmulator {
        machineInstruction.cycle += 1
    ...
    if nextAction == .fetchNext || nextAction == .fetchPrefix {
      ...
      if nextAction == .fetchNext && !interrupts.isEmpty {
        ...
        if emulator is LR35902.Emulation.not_implemented {  // TODO: Remove this check once fully migrated.
          machineInstruction.microcode = LR35902.InstructionSet.microcodes[specIndex]
          machineInstruction.instructionEmulator = nil
        } else {
          machineInstruction.instructionEmulator = emulator
          machineInstruction.microcode = nil
        }
      }
    }
