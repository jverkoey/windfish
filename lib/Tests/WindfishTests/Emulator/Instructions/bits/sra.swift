...
extension InstructionEmulatorTests {
  ...
  func test_sra_r() {
    for spec in LR35902.InstructionSet.allSpecs() {
      guard case .cb(.sra(let register)) = spec, let emulator = LR35902.Emulation.sra_r(spec: spec) else { continue }
  ...
  func test_sra_hladdr() {
    for spec in LR35902.InstructionSet.allSpecs() {
      guard let emulator = LR35902.Emulation.sra_hladdr(spec: spec) else { continue }
      InstructionEmulatorTests.testedSpecs.insert(spec)
      for (name, testCase) in TestCase.testCases {
