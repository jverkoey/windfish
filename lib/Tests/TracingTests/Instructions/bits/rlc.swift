...
extension InstructionEmulatorTests {
  private struct TestCase {
    ...
    static let testCases: [String: TestCase] = [
      ...
      "1000_0000":  .init(value: 0b1000_0000, fc: false, result: .init(fc: true,  fz: false, value: 0b0000_0001)),
  ...
  func test_rlc_r() {
    for spec in LR35902.InstructionSet.allSpecs() {
      guard case .cb(.rlc(let register)) = spec, let emulator = LR35902.Emulation.rlc_r(spec: spec) else { continue }
  ...
  func test_rlc_hladdr() {
    for spec in LR35902.InstructionSet.allSpecs() {
      guard let emulator = LR35902.Emulation.rlc_hladdr(spec: spec) else { continue }
      InstructionEmulatorTests.testedSpecs.insert(spec)
      for (name, testCase) in TestCase.testCases {
